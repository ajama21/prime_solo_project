...
function* updateDriverDetails(action) {
  try {
    ...
    const response = yield axios.put('/api/driver/' + action.payload.id, action.payload, config);

    yield put({ type: 'FETCH_DRIVER_DETAILS', payload: action.payload.id });
  } catch (error) {
    console.log('User get request failed', error);
  }
}
...
function* deleteDriver(action) {
  try {
    ...
    const response = yield axios.delete('/api/driver/' + action.payload, config);
...
function* driversaga() {
  ...
  yield takeLatest('UPDATE_DRIVER_DETAILS', updateDriverDetails);
